...
def test_process_data(sample_data, expected_df):
    ...
    expected_columns = [
        'electric_bike', 'departure_hour', 'membership_cat_annual_pass',
        'membership_cat_other', 'membership_cat_pay_per_ride',
        'membership_cat_community_pass', 'membership_cat_unknown',
        'membership_cat_monthly_pass', 'duration_min', 'departure_temperature_c'
    ]

    assert all(col in processed_df.columns for col in expected_columns), "Not all expected columns are in the processed DataFrame"
